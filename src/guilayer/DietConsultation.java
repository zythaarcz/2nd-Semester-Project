	...
	public DietConsultation() {
		...
		chooseButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				calendarWindow.setLocation(dateTextField.getLocationOnScreen().x,
						dateTextField.getLocationOnScreen().y + dateTextField.getHeight());
					...
					if (!isNumeric(reason)) {
						if (LocalDate.now().isBefore(localDate)) {
							...
							if (wasCreated) {
								...
								homepageCustomer.setLocationRelativeTo(null);
								homepageCustomer.setVisible(true);
								dispose();
							} else {
								JOptionPane.showMessageDialog(contentPane,
										"We are sorry, but another customer booked the meeting before you.\nTry another date.");
							}
						} else {
